...
func (_q *DocumentQuery) QueryUser() *UserQuery {
...
func (_q *DocumentQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *DocumentQuery) FirstIDX(ctx context.Context) uint {
...
func (_q *DocumentQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *DocumentQuery) OnlyIDX(ctx context.Context) uint {
...
func (_q *DocumentQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (_q *DocumentQuery) IDsX(ctx context.Context) []uint {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *DocumentQuery) WithUser(opts ...func(*UserQuery)) *DocumentQuery {
	query := (&UserClient{config: _q.config}).Query()
...
//		GroupBy(document.FieldDocumentName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Document.Query().
//		Select(document.FieldDocumentName).
//		Scan(ctx, &v)
...
func (_q *DocumentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Document, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Document, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DocumentQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Document, init func(*Document), assign func(*Document, *User)) error {
	ids := make([]uint, 0, len(nodes))
	nodeids := make(map[uint][]*Document)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *DocumentQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(document.Table, document.Columns, sqlgraph.NewFieldSpec(document.FieldID, field.TypeUint))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(document.FieldUserID)
		}
	}
